...
$script:securitylogPath = "$LeslieAppBasePath\Logs\security.log";
...
try
{
    Add-Type -Path "$LeslieAppBasePath\Resources\dll\log4net.dll";
}
...
$log4netAppConfigFile = New-Object System.IO.FileInfo "$LeslieAppBasePath\core\Log4netConfig\app.config"
...
if($log4netAppConfigFile.Exists) { 
    Add-SecurityLog "log4net app.config file has been found at $LeslieAppBasePath\core\Log4NetConfig\app.config, and is now about to be initialised...."
    [log4net.Config.XMLConfigurator]::Configure($log4netAppConfigFile)
}
else{ 
    # Add-SecurityLog ("File: $LeslieAppBasePath\core\Log4NetConfig\app.config could not be found. Log4Net initialisation to MSSQL will now abort.")
}
